...
def search_anime_flow(args):
    """Flow for searching and selecting an anime with incremental search.

    Implements incremental word addition: starts with 3 words and progressively adds
    words until results ≤ 5 or all words are used.

    Supports navigation between result sets if multiple iterations returned results.
    ...
    if cache_data:
        ...
        # Not in cache or expired: search scrapers using incremental search
        search_state, titles_with_sources = incremental_search_anime(query)

        if not titles_with_sources:
            return None, None, None  # No results found at all

        # Menu loop: allow navigation between result sets
        while True:
            # Get current word count for display
            current_result_set = search_state.get_current()
            assert current_result_set is not None  # Should always be valid
            word_count_display = f"({current_result_set.word_count} palavras: {len(current_result_set.results)} resultados)"

            # Show menu with navigation options
            selected_anime_with_source = menu_navigate(
                list(titles_with_sources),
                msg=f"Escolha o Anime. {word_count_display}",
