...
class TestDattebayoSearch:
    ...
    @patch("scrapers.plugins.dattebayo.fetch")
...
class TestDattebayoSigning:
    ...
    @patch("scrapers.plugins.dattebayo.fetch")
    def test_sign_video_url(self, mock_fetch):
        mock_fetch.return_value = _json_response([{"ads": "OK", "publicidade": "?sig=abc"}])
    ...
    @patch("scrapers.plugins.dattebayo.fetch")
    def test_resolve_signed_video_url_fallback(self, mock_fetch):
        ...
        mock_fetch.side_effect = get_side_effect

        resolved = resolve_signed_video_url(episode_url)
...
class TestDattebayoPlayerSrc:
    ...
    def test_search_player_src_stores_signed_url(self, mock_resolve):
    ...
    @patch("scrapers.plugins.dattebayo.resolve_signed_video_url")
    def test_search_player_src_raises_when_unresolved(self, mock_resolve):
